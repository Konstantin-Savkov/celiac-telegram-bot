...
let TelegramService = TelegramService_1 = class TelegramService {
    ...
    async handleUserState(chatId, from, text, state) {
    ...
    async handleStart(chatId, from) {
        ...
        const welcomeText = `🌾 Здравствуйте, ${displayName}!\n\n` +
    ...
    async handleAdminCommand(chatId, from) {
    ...
    async sendMainMenu(chatId, telegramId, text) {
    ...
    async showInfoDetail(chatId, id) {
    ...
    async showAllowedCategories(chatId) {
    ...
    async showAllowedByCategory(chatId, category) {
    ...
    async showForbiddenCategories(chatId) {
    ...
    async showForbiddenByCategory(chatId, category) {
    ...
    async showChecklistCategories(chatId) {
    ...
    async showChecklistByCategory(chatId, category) {
        const items = await this.prisma.shopping_checklist.findMany({
            where: { category },
            orderBy: { order: 'asc' },
        });
    ...
    async startForumSubmission(chatId, from) {
    ...
    async submitForumPost(chatId, from, productName, description) {
        ...
        await this.sendMessage(chatId, `✅ Ваше предложение "отправлено на модерацию!\n\n` +
            `📦 Продукт: *${productName}*\n` +
    ...
    async showApprovedPosts(chatId, offset) {
    ...
    async startChatWrite(chatId, from) {
    ...
    async saveChatMessage(chatId, from, message) {
    ...
    async sendAdminPanel(chatId) {
        ...
        await this.sendMessage(chatId, `⚙️ *Панель администратора*\n\nОжидают модерации: ${pendingCount}`, {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: [
    ...
    async showPendingPosts(chatId, offset) {
        const posts = await this.prisma.safe_product_post.findMany({
        ...
        const text = `📝 *Предложение ${offset + 1} из ${total}*\n\n` +
    ...
    async moderatePost(chatId, from, postId, status) {
        const isAdmin = await this.isAdmin(from.id);
        if (!isAdmin) {
    ...
    async showAdminStats(chatId) {
        const [userCount, messageCount, approvedCount, pendingCount, allowedCount, forbiddenCount] = await Promise.all([
    ...
    async answerCallbackQuery(callbackQueryId) {
        try {
            await axios_1.default.post(`${this.apiUrl}/answerCallbackQuery`, {
